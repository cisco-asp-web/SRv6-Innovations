...
In the previous section, we validated the SRv6 control plane, confirming that color-based SR policies, Binding SIDs, and segment lists were correctly programmed between the xrd01 and xrd07 sites. We now move to data-plane validation, focusing on how traffic originating from app-container-01 is actually forwarded across the network toward the app-container-07. Using SRv6 color-based policies, packets are steered along explicitly engineered paths, independent of default IGP routing. Two traffic intents are validated: a bulk-transfer path and a low-latency path, each traversing a different set of intermediate nodes. By observing packet forwarding and captures along the path, we validate how control-plane intent is enforced in the data plane through SRv6 Binding SIDs and segment lists.

**Validate bulk traffic takes the non-shortest path: xrd01 -> 02 -> 03 -> 04 -> xrd07** 

In SRv6 Traffic Engineering, uSIDs allow a source node (ingress PE, xrd01 in this case) to explicitly program a packet’s path through a domain by enforcing a sequence of intermediate waypoints or links.
...
1. Using the Visual Code extension, ssh to the **app-container-01's** shell and run a ping to the bulk transport destination IPv4 address on xrd07. We will capture and analyze this traffic at multiple points across the network.

   ```
    ping 40.0.0.1 -i .5
   ```

![xrd02 ping](../topo_drawings/lab2-xrd02-ping.png)

  Launch an edgeshark capture on the **xrd01** container interface Gig0/0/0/0 to inspect the traffic.

![London gi0/0/0](../topo_drawings/lab2-xrd-edgeshark-g0.png)

  Observing the ICMP traffic exchanged between the and app-container-07s, the echo request is a standard IPv4 packet sourced from 10.101.1.2 and destined for 40.0.0.1, with a measured round-trip time of approximately 120 ms. This latency matches the delay values that were intentionally introduced on the links in Lab 1, confirming that traffic is traversing the expected network path.
...
4. Launch an edgeshark capture on container **xrd03** interface Gig0/0/0/0 to inspect the traffic.
...
5. Launch an edgeshark capture on container **xrd04** interface Gig0/0/0/0 to inspect the traffic.

  ![Zurich ingress Wireshark Capture](../topo_drawings/lab2-xrd04-wireshark-g0.png)

Here, the xrd04 (XRD04 – 4444) receives a packet whose IPv6 destination address contains the microSID of the final destination (7777) rather than its own, and therefore forwards the packet as standard IPv6 traffic toward the endpoint.
...
6. Launch an edgeshark capture on container **xrd07** interface Gig0/0/0/1 to inspect the traffic sent by **xrd04**.

  ![Rome ingress Wireshark Capture](../topo_drawings/lab2-xrd07-wireshark-g1.png)

When xrd07 receives the packet with an IPv6 destination address of fc00:0:7777:e007::, the active microSID (7777) matches xrd07’s own uSID, indicating that the packet has reached its final SRv6 endpoint. At this stage, all transit microSIDs have already been consumed, and no further traffic steering is required. xrd07 therefore executes the endpoint service behavior associated with e007 (for example, uDT4/uDT6: More information [here](https://www.ciscolive.com/c/dam/r/ciscolive/emea/docs/2025/pdf/BRKSPG-2203.pdf) and [here](https://datatracker.ietf.org/doc/html/rfc8986#name-enddt6-decapsulation-and-sp)), decapsulates the packet as needed, and forwards the inner payload according to the local routing or VRF configuration.



7. Launch an edgeshark capture on container **xrd07** interface Gig0/0/0/0 to inspect the traffic.

  ![Rome Wireshark Capture](../topo_drawings/lab2-xrd07-wireshark-g1.png)

At xrd07, the SRv6 transport header has been fully processed and removed, and the packet is delivered to the endpoint service. As shown in this capture, only a standard IPv4 ICMP packet remains, sourced from 10.101.1.2 and destined for 40.0.0.1, confirming that the outer SRv6 header been decapsulated. This behavior corresponds to the execution of the SRv6 uDT4 endpoint function, which forwards the packet into the correct routing context. The packet is then forwarded within the Carrots VRF, completing the end-to-end SRv6 packet walk from xrd01 to xrd07.

<br><br>

**Validate low latency traffic takes the path: xrd01 -> 05 -> 06 -> xrd07**
...
2.  Let's test and validate that our SRv6 TE policy is applied on **xrd01**. From **app-container-01** we will ping **xrd07's** low latency IPv4 destination:
    ```
    ping 50.0.0.1 -i .5
    ```
